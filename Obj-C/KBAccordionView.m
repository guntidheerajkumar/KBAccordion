...
@implementation KBAccordionView {
...
-(id)initWithSections:(NSArray *)sections andSectionStyle:(KBAccordionSectionStyle *)sectionStyle {
    ...
    if (self) {
        ...
        for (NSUInteger i = 0; i < [sections count]; i++) {
            KBAccordionSection*currentSection = [sections objectAtIndex:i];
            KBAccordionSectionView *sectionView =
            [[KBAccordionSectionView alloc] initWithTitle:currentSection.title
...
-(void)addSection:(KBAccordionSectionView*)newSection {
...
-(void)requestNotificationWhenSectionHeightChanges:(KBAccordionSectionView *)sectionView {
    [sectionView addObserver:self forKeyPath:SECTION_HEIGHT_GETTER options:0 context:nil];
}

-(void)dealloc {
    for (KBAccordionSectionView *section in _sectionViews){
        @try {
...
-(void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:SECTION_HEIGHT_GETTER]) {
        KBAccordionSectionView *changedSection = (KBAccordionSectionView *) object;
...
-(void)makeSureSomeOfTheExpandedContentIsVisible:(KBAccordionSectionView *)expandedSection {
